...
# ------------------------------------------------------- Почему нет фильмов
# Жалоба «в кино только посты» проверялась только глазами по стене, а SSH нет.
# Печатаем причину прямо здесь: набирать команды в VNC-консоли неудобно, а без причины
# любая гипотеза остаётся гаданием (три подряд уже оказались неверными).
echo
echo "==> Фильмы и клипы Кино, последние записи журнала:"
grep -hiE 'Видео-репост|Клип [0-9]+|нарезка клипов' \
    /opt/news-rewriter/logs/publishing.log /opt/news-rewriter/logs/app.log 2>/dev/null \
    | tail -20 || echo "    записей нет — джоб ни разу не дошёл до фильма"

...
class WatermarkConfig:
    ...
    # Накладывать ли логотип на ФОТО. False — фото без лого (режим «простое медиа»
    # Новостей: только заголовок-хук, без вотермарка/фейда, запрос 2026-07-27).
...
class ScheduleConfig:
    ...
    # Публикуем только СВЕЖИЕ посты: если самый свежий в очереди старше этого окна —
    # не публикуем ничего, а старьё протухает (status='expired'). Убирает "публикуются
    # старые новости" (запрос пользователя 2026-07-05). Считается по created_at поста.
    publish_freshness_hours: int = 6

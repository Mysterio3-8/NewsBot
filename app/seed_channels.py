...
DAILY_PLAN = {
    ...
    # Клипов 1 (ТЗ владельца 2026-09-09 «1 фильм 2 поста, 1 клип» — беречь единственный
    # живой токен). Считается ДВАЖДЫ: столько хуков просим у LLM и столько кусков режем
    # из фильма — оба места читают эту же настройку, поэтому менять её достаточно здесь.
    "daily_clip_count": 1,
    # Рерайт-посты: 2/сутки (ТЗ владельца 2026-09-09 «1 фильм 2 поста, 1 клип»).
    # Причина урезки не в антибане как таковом, а в том, что живой личный токен остался
    # ОДИН и получен в обход официального приложения — чем реже он дёргается, тем дольше
    # живёт. Итого записей Кино в сутки: 4 (фильм + клип + 2 поста) вместо шести.
    # Окно 09:00–24:00 = 900 мин, постов два → один промежуток, потолок 900. 300–420
    # разводит их по окну и оставляет место фильму и клипу.
    "max_posts_per_day": 2,
    ...
    # брать и читать посты, публиковать не надо». Источники Telegram продолжают
    # читаться — выключена ровно публикация.
    "tg_publish_enabled": False,
...
def seed_music_news(repo: Repository) -> None:
    ...
    Публикуем ЛИЧНЫМ токеном (`VK_USER_TOKEN`), а не групповым: отдельного
    `VK_GROUP_TOKEN_MUSIC` так и не завели, и канал месяц простоял выключенным, ожидая
    его. Личный токен у нас один на все софты и он же грузит медиа — публикация им
    ничего не добавляет к нагрузке на аккаунт сверх той, что и так есть.

    enabled_on_create=True (ТЗ владельца 2026-09-09 «Музыка 1 плейлист + 1 трек +
    1 новость»). Состояние уже существующего канала сид по-прежнему не трогает —
    выключить его можно из бота, и сид это переживёт."""
    settings = ChannelSettings(**MUSIC_NEWS_SETTINGS)
    channel = _ensure_channel(
        repo,
        "Infinity Music - новость дня",
        vk_destination=MUSIC_NEWS_VK_GROUP,
        vk_token_env="VK_USER_TOKEN",
        settings_json=settings.to_json(),

...
DAILY_PLAN = {
    ...
    # 1 клип на фильм (ТЗ 2026-08-20). Считается ДВАЖДЫ: столько хуков просим у LLM и
    # столько кусков режем из фильма — оба места читают эту же настройку, поэтому
    # менять её достаточно здесь. Итого записей Кино на стене в сутки: 5
    # (3 поста + фильм + клип) вместо семи.
    "daily_clip_count": 1,
    # Рерайт-посты: 3/сутки (ТЗ 2026-08-20: «1 фильм 1 клип 3 поста»).
    # Окно Кино 09:00–24:00 = 900 мин, постов три → два промежутка, потолок среднего
    # 450 мин. 300–420 даёт среднее 360: посты расходятся по всему окну и оставляют
    # промежутки под фильм и клипы, но последний успевает выйти до полуночи.
    "max_posts_per_day": 3,

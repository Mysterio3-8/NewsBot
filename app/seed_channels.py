...
DAILY_PLAN = {
    ...
    # ТЗ владельца 2026-08-14: «кино днём». Кино — единственный софт с дневным окном,
    # остальные три уведены в ночь, чтобы не отбирать у фильма слот загрузки медиа
    # (аккаунт один на всех, зазор 60 минут). Часы в МСК: тишина 21:00–11:00, то есть
    # публикации идут с 11:00 до 21:00 — как раз вокруг старта фильма (08:00 UTC = 11 МСК).
    "quiet_start_hour": 21,
    "quiet_end_hour": 11,
...
NEWS_ANTIBAN = {
    ...
    # Часы в МСК (rate_guard.is_quiet_now). Тишина 11:00–21:00 = работа ночью.
    "quiet_start_hour": 11,
    "quiet_end_hour": 21,

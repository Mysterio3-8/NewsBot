...
# сами токены живут в .env (инвариант проекта). Балансер берёт наименее нагруженный
# сегодня токен, поэтому объём канала делится между аккаунтами, а не выжигает один.
# VK_PHOTO_UPLOAD_TOKEN_2 добавлен 2026-08-03 после бана — второй аккаунт, общий для
# обоих каналов (счётчики балансера глобальные по имени переменной, поэтому суммарная
# нагрузка на этот аккаунт считается корректно через оба канала).
NEWS_UPLOAD_POOL = ["VK_PHOTO_UPLOAD_TOKEN", "VK_PHOTO_UPLOAD_TOKEN_2"]
KINO_UPLOAD_POOL = ["VK_PHOTO_UPLOAD_TOKEN_KINO", "VK_PHOTO_UPLOAD_TOKEN_2"]

# Потолок загрузок на ОДИН личный токен в сутки. Бан 2026-07-02 случился примерно на 12
# публикациях подряд, поэтому держим ниже: 10 загрузок на аккаунт в сутки.
VK_TOKEN_DAILY_CAP = 10
...
DAILY_PLAN = {
    "daily_video_count": 2,
    # Зазор в МИНУТАХ перекрывает daily_video_min_gap_hours. 11 ч между двумя фильмами
    # разносит их на противоположные половины суток (напр. 11:00 и 22:00 МСК).
    "daily_video_min_gap_hours": 11,
    "daily_video_min_gap_minutes": 660,
    "daily_clip_count": 2,
    # Рерайт-посты: 4/сутки. Интервал 4-7 ч случайный — при 24-часовом окне это ровно
    # 4 поста, разбросанных по дню, и они попадают в промежутки между фильмами.
    "max_posts_per_day": 4,

...
def _ensure_channel(repo: Repository, name: str, **fields) -> Channel:
    existing = next((c for c in repo.list_channels() if c.name == name), None)
    if existing is not None:
        print(f"Канал «{name}» уже есть (id={existing.id})")
        return existing
    channel = repo.create_channel(name=name, **fields)
...
def seed_cinema(repo: Repository) -> None:
    """Канал 2 — Кино. Публикация только в VK (240120678), TG добавим позже. Фильтр off
    (лить всё подряд), лимит 4/день. Источник — VK «Кинопремьеры 2026» (58170807)."""

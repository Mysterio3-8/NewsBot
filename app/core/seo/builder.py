...
class SeoProfile:
    ...
    """Постоянные запросы САМОГО сообщества: «фильмы смотреть онлайн», «новости сегодня».

    ТЗ владельца 2026-08-11: «чтобы люди писали кино или фильмы — и выдавал мой канал».
    Это другой сорт ключей, чем `search_phrases`: те приводят человека, который ищет
    конкретное имя («Роберт Дауни»), а эти — того, кто ищет ТЕМУ. По теме ищут на
    порядок чаще, и такой запрос не зависит от того, нашлись ли в тексте имена
    собственные, поэтому фразы идут в КАЖДОМ описании ролика."""
...
def build_search_line(text: str, profile: SeoProfile) -> str:
    ...
    # Постоянные запросы сообщества идут ВСЕГДА — даже когда имён собственных в тексте
    # не нашлось. Это и есть страховка «канал должен находиться по слову кино», которая
    # не зависит от того, что написал источник.
    phrases.extend(phrase for phrase in profile.channel_phrases if phrase.strip())

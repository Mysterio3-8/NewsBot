...
PROXY_ENV = "YT_PROXY"
"""Прокси для запросов к YouTube (`socks5://127.0.0.1:PORT`).

Появился 2026-08-14, когда куки перестали помогать: Google отозвал их после первого же
использования с серверного IP, а новый экспорт умирал так же. Живой перебор пяти
VPN-выходов показал главное — **дело в IP, а не в куках**: на шведском выходе YouTube
отдаёт форматы БЕЗ всяких куки (22 формата, 15 с видео), на остальных четырёх — «Sign in
to confirm you're not a bot», тоже независимо от куки.

Поэтому прокси теперь основной способ, а куки — необязательный довесок. Это ещё и
безопаснее: куки залогиненного аккаунта на сервере рискуют самим аккаунтом, а прокси
ничем не рискует.

⚠️ Через прокси ходит ТОЛЬКО yt-dlp. VK и Telegram обязаны идти напрямую: смена страны
у запросов к VK — верный способ поймать проверку безопасности на аккаунте."""

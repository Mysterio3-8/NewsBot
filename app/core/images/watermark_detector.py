"""Проверка чужого водяного знака/логотипа на фото поста перед публикацией
(запрос пользователя 2026-07-05). Если знак можно убрать простой обрезкой
верхнего и/или нижнего края — используем ЭТО ЖЕ фото, обрезав знак (запрос
пользователя: "хочу чтобы ты в точь в точь находил оригинальное фото только
без чужого монтажа и ватермарков"), не сток-фолбэк. Если знак по центру/на
самом сюжете и обрезкой его не убрать — фото не годится, пайплайн переходит
на сток (Pexels/Unsplash/Pixabay), как и раньше."""
from __future__ import annotations

import logging
from pathlib import Path

from app.core.llm.client import LLMClient, LLMUnavailableError

logger = logging.getLogger("monitoring")

WATERMARK_LOCATE_PROMPT = (
    "Посмотри на фото. Есть ли на нём ИМЕННО водяной знак или логотип ДРУГОГО "
    "медиа/канала — фирменный значок, буква-логотип в углу, полупрозрачная "
    'подпись поверх фото, надпись вида "Источник: ..." или название другого '
    "паблика/канала?\n"
    "НЕ считай водяным знаком: случайные цифры, номера кадра, мелкие подписи "
    "фотобанка, дату или любой другой мелкий текст, который не похож на "
    "фирменный логотип/бренд конкретного медиа. Если сомневаешься, есть ли это "
    "настоящий вотермарк — отвечай, что его нет.\n"
    "Если знака НЕТ — ответь одним словом: нет\n"
    "Если знак есть и находится ТОЛЬКО у верхнего края фото (полоса или значок "
    "сверху, можно обрезать верхнюю часть и убрать) — ответь: верх\n"
    "Если знак есть и находится ТОЛЬКО у нижнего края — ответь: низ\n"
    "Если есть два знака — один у верхнего края, другой у нижнего — ответь: верх,низ\n"
    "Если знак находится по центру фото, поверх главного объекта, или его нельзя "
    "убрать простой обрезкой сверху/снизу — ответь: не убрать"
)

_EDGE_REGIONS = frozenset({"top", "bottom"})

TEXT_HEAVY_PROMPT = (
    "Посмотри на изображение. Это скриншот, документ, объявление, мем или картинка, "
    "ПОКРЫТАЯ большим количеством текста — много строк/абзацев/крупных надписей прямо "
    "на самом изображении?\n"
    "Речь НЕ про мелкую подпись или логотип в углу, а про изображение, где текст "
    "занимает заметную часть кадра.\n"
    "Если текста много (заголовок, наложенный сверху, смешается с ним и станет "
    "нечитаемым) — ответь одним словом: да\n"
    "Если это обычное фото/сюжет, где текста мало или нет — ответь: нет"
)


def image_has_heavy_text(client: LLMClient, image_path: Path) -> bool:
    """True — на изображении много собственного текста (скриншот/документ/мем/объявление),
    заголовок-хук поверх смешается в кашу, ставить его не надо (ТЗ 2026-07-27).
    FAIL-OPEN: если vision недоступен — False (ставим заголовок как обычно; большинство
    новостных фото — сюжетные, без сплошного текста)."""
    try:
        response = client.generate_vision(TEXT_HEAVY_PROMPT, image_path)
    except Exception:
        logger.info("Проверка текста на фото недоступна для %s — считаем, что текста мало", image_path)
        return False
    return response.strip().lower().startswith("да")


def locate_foreign_watermark(client: LLMClient, image_path: Path) -> set[str] | None:
    """Возвращает множество краёв, которые нужно обрезать, чтобы убрать чужой
    знак: {"top"}, {"bottom"}, {"top", "bottom"} или пустое множество (знака нет).
    None — фото использовать нельзя (знак неубираемой обрезкой ИЛИ проверку не
    удалось выполнить).

    FAIL-CLOSED (явное требование пользователя 2026-07-11: "нельзя брать фото если
    на них есть чужие вотермарки"): если vision недоступен/упал (напр. Groq 429),
    мы НЕ знаем, есть ли на фото чужой знак — поэтому фото НЕ используем (None →
    пайплайн уходит на сток), а не пропускаем непроверенным. Лучше нейтральный
    сток, чем чужой вотермарк в публикации."""
    try:
        response = client.generate_vision(WATERMARK_LOCATE_PROMPT, image_path)
    except LLMUnavailableError:
        logger.warning(
            "Проверка водяного знака недоступна для %s — фото НЕ используется (fail-closed), уходим на сток",
            image_path,
        )
        return None
    except Exception:
        logger.exception("Проверка водяного знака упала для %s — фото НЕ используется (fail-closed)", image_path)
        return None

    normalized = response.strip().lower()
    if normalized.startswith("нет"):
        return set()
    if "не убрать" in normalized:
        return None

    regions = {region for region in ("верх", "низ") if region in normalized}
    if not regions:
        # Неожиданный ответ модели — безопаснее считать знак неубираемым, чем
        # рискнуть оставить его на фото.
        return None

    return {"top" if region == "верх" else "bottom" for region in regions}
